var jsKeyboard = {
    ...
    compileData: function (data) {
        $('#debugger').val(JSON.stringify(data));
        console.log(data);
        jsKeyboard.data.push(data);
        if (data.type == 'down' && data.key == 'enter'){
            console.log('SUBMITTED',jsKeyboard.data);
            jsKeyboard.data = [];
        }
    },
    data: [],
    addKeyDownEvent: function () {
        $("#keyboardCapitalLetter > div.button, #keyboardSmallLetter > div.button, #keyboardNumber > div.button, #keyboardSymbols > div.button")
            .bind('mousedown', (function (e) {
                    ...
                    var key = onclick.substring(onclick.indexOf('(') + 1, onclick.indexOf(')'));

                    if (!key) {
                        key = onclick.substring(onclick.indexOf('.') + 1, onclick.indexOf('('));
                    }
                    var data = {
