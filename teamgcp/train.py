...
def selectFeatures(combined, cols):
    ...
    shortlist1 = pd.DataFrame({'feature': cols[2:], 'importance': rfe.ranking_}).sort_values(by=['importance'],
                                                                                             ascending=True)
    ...
    shortlist2 = pd.DataFrame({'feature': train_features, 'importance': model.feature_importances_}).sort_values(
...
def getTrainRecords(txt):
    ...
    return dataset, labels, shortlist
